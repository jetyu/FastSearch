...
在搜索结果页面中，可以通过快捷菜单切换搜索引擎并复用当前关键词；也可以选中文字后使用划词搜索或搜索弹窗发起搜索。脚本支持网址管理、自定义分类与排序、整份配置备份与恢复、新标签页打开设置，以及自动 / 深色 / 浅色外观。
...
| 划词搜索 | `/config/toolbar` | 编辑划词搜索入口、拖拽排序、显示隐藏                                               |

“配置”和“编辑”共享菜单草稿；“划词搜索”单独编辑和保存。所有页面都在脚本内运行，不需要部署或打开 GitHub 配置网站。

各 Tab 底部仅保留 **取消 / 保存**。取消会恢复到上次保存的内容；“配置”和“编辑”的菜单草稿会一起恢复，划词搜索的草稿独立处理。

“编辑”页的 JSON 编辑器上方提供 **清除网址管理配置**。确认后立即删除已保存的自定义菜单配置及当前菜单草稿，恢复脚本内置网址，同步更新“配置”和“编辑”页，无需再点保存。划词搜索、其他设置和图标缓存独立保留。需要保留自定义配置时，可先导出整份备份。
...
- 划词搜索默认入口依次为 Google、百度、Google翻译、ChatGPT。
...
5. 按下表验证。打开搜索弹窗的方法是：选中页面文字，在划词搜索中点击“更多”图标。
...
| 普通点击划词搜索中的搜索图标                              | 沿用原有行为，在新标签页打开 |

每次修改源码后，重新执行 `corepack pnpm build:script`，更新脚本管理器中的代码，并刷新待测页面。

#### 网址管理对话框测试（1.6.0）

1. 安装 `output/index.user.js`，刷新搜索结果页，打开“设置 → 网址管理 → 打开”，确认标题显示 `1.6.0`，有“配置 / 编辑 / 划词搜索”三个 Tab，各 Tab 底部只有“取消 / 保存”。
...
5. “划词搜索”单独保存，不会同时保存搜索菜单的草稿。点击“取消”会恢复当前范围上次保存的内容，另一个范围的草稿保留；关闭对话框时，有未保存的修改会提示是否放弃。
...
8. 在“编辑”中点击“清除网址管理配置”，先取消确认，检查原配置和草稿保留；再次点击并确认，检查菜单及编辑器恢复内置网址，刷新后仍使用内置网址，划词搜索和其他设置保留。
